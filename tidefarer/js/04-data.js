/* =====================================================================
   ...
   ===================================================================== */
const ITEMS = {
  wood:{name:'Wood', desc:'Fresh-cut timber.', cat:'material'},
  stone:{name:'Stone', desc:'Solid island granite.', cat:'material'},
  fish:{name:'Fish', desc:'Eat to restore 12 HP.', use:'heal', heal:12, cat:'food'},
  wheat:{name:'Wheat', desc:'Eat to restore 10 HP.', use:'heal', heal:10, cat:'food'},
  seed:{name:'Wheat Seeds', desc:'Plant in tilled soil at the farm.', cat:'material'},
  mushroom:{name:'Bluecap', desc:'A glowing mushroom from the Whisperwood.', cat:'material'},
  potion:{name:'Ember Tonic', desc:'Restores 30 HP.', use:'heal', heal:30, cat:'food'},
  goo:{name:'Slime Goo', desc:'A quivering green glob. Render three down at a brewing cellar and they thicken into a tonic.', cat:'material'},
  charm:{name:'Ember Charm', desc:'+3 damage to every attack.', cat:'charm'},
  skymap:{name:'Cloud-Chart', desc:'A chart of the wind-roads, kept in the Broken Crown. Show it to Ashwing and he will bear you between the isles - to Windsurf, back to the Cloudreach, or on to the Sunward Isle.', cat:'key'},
  veilrune:{name:'Hush-Frost Spellbook', desc:'An ancient spell book of the old royal line, its pages hush-frost that will not thaw, scored in a royal script only a scholar could read. The Rimebound was set to guard it long ago; no one living knows what its workings do. Carry it to your brother.', cat:'key'},
  ...
  sealtome:{name:"The Founders' Sealing Book", desc:'A heavy book bound in tide-worn hide, drawn deep in the old royal script and guarded since the founders by the Tideward Guardian. It holds the one working the old line feared enough to seal away: a binding strong enough to cage the shadow itself. You cannot read it - carry it to your brother Leo, the only scholar who can, and the only one who could ever cast it.', cat:'key'},
  rivenedge:{name:'Rivenedge Axe', desc:'A dungeon-forged greataxe. Fells the blue-black ironwood that walls the old paths - and bites through any ordinary pine in a swing or two.', cat:'tool'},
  cragbreaker:{name:'Cragbreaker Pick', desc:'A dungeon-forged pick of blackened steel. Shatters the violet basalt that seals the deep ways - and splits common stone far faster.', cat:'tool'},
  delvebreaker:{name:'Delvebreaker Pick', desc:'A pick of star-dark iron from the Undermaw\'s deep hoard. Shatters the bright-violet wardstone Vath raised to cut Barik off - and splits common basalt the same.', cat:'tool'},
  cograzor:{name:'Cograzor Pick', desc:'A mill-forged pick hafted from a broken gear-tooth. Breaks the rust-red slagiron that seals the mill-deep ways - and tears through common stone faster still.', cat:'tool'},
  emberbreaker:{name:'Emberbreaker Pick', desc:'A pick quenched in the Ashen Forge, its head still smoking. Shatters the molten emberstone that fused the old vaults shut - and any lesser stone gives at a touch.', cat:'tool'},
  reefpick:{name:'Coast-Warden Pick', desc:'The storm-coast\'s own pick, knapped from the green reef-stone it breaks and warded by the spirit that guarded it. Shatters the reef-stone the castaways raised to seal their dead - and any lesser stone gives at a touch.', cat:'tool'}
};
const SKILLS = {
...
function spawnNPCs(){
  G.npcs = [
    ...
    makeNPC('bram','Bram the Smith',59,57,{skin:'#d9a06a',hair:'#3a2a1c',shirt:'#8f4a3a',pants:'#3a3a40',hairstyle:'bald',beard:'#2c1f14',apron:'#4a3322',size:1.06,build:{w:1.2,head:0.88}},
      ['A dull blade is a heavy blade.','Stone from the north road takes an edge best.'],0.4),
...
const MOBDEF = {
  ...
  brigand:{hp:240, dmg:22, speed:2.4, aggro:9, xp:180, gold:[18,34], lvl:7, name:'Pinewood Brigand'},  // slow, lumbering camp guards - you can sprint the gaps and beat them to the cache
