...
function boot(){
  buildTiles(); buildFringes(); buildShoreAO(); buildShoreSurf(); buildCorners(); buildSprites(); buildIcons(); buildExtraSprites();
  weatherAll();
  G.worldId='isle';
  genIsleAll();
  ...
  { const _db=document.getElementById('deflectBtn'); if(_db) pressable(_db, ()=>{ if(typeof tryAirSlash==='function') tryAirSlash(); }); }   // the 3rd button is now Air Slash
  buildHotbar(); refreshUI();
  // controls blurb on title
  document.getElementById('ovControls').innerHTML = isTouch
    ? '<b>Left thumb</b> - joystick to move · <b>⚔</b> - attack / gather · <b>⤸</b> - dodge roll · <b>◆</b> - air slash · <b>green button</b> - talk, fish, harvest · <b>parry</b>: swing on a foe\'s white flash'
    : '<b>Click</b> to walk, gather, talk, fight - or <b>WASD</b> · <b>Space</b>/<b>K</b> attack · <b>Ctrl</b>/<b>L</b> dash · <b>O</b> air slash · <b>E</b> interact · <b>parry</b>: attack on a foe\'s white flash · in dialog, <b>Enter</b>/<b>number keys</b> · <b>gamepad supported</b>';
...
function startIntro(){
  ...
  const p2=()=>{
    setDialog('“Your ship went down on the strait in the night - I watched its lanterns swallowed from the headland. Dark things have stirred off our shores of late, and the crossing has turned deadly; yours was only the last hull to try it.” <i>The old woman studies the pale mask over your face, and thinks better of asking.</i> “Keep the mask, if it steadies you. Do you know it - your ship, your name, where you were bound?”',
...
function afterIntro(){
  // She stays standing where she spoke to you - no jarring snap back to the market. Her home post
  // (hx/hy) is still the village, and her wander was left at her spawn value, so she simply STROLLS
  // back on her own over the next few seconds instead of teleporting. (See the wander tick in
  // 09-gameplay: it walks her toward a target near hx/hy.)
  const greeter=(G.npcs||[]).find(n=>n.id==='maren');
  if(greeter && greeter._home){ greeter.wander=greeter._home.wander; greeter.face={x:0,y:1}; delete greeter._home; }
