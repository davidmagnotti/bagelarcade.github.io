...
function pickClickTarget(wx,wy){
  ...
  for(const b of G.decor){
    ...
    else if(b.kind==='signalbeacon') cand.push({type:'inter',x:b.x,y:b.y,r:1.4,range:2.4,go:()=>{ if(typeof signalAshwing==='function') signalAshwing(b); }});
    else if(b.kind==='fastexit') cand.push({type:'inter',x:b.x,y:b.y,r:1.5,range:2.4,go:()=>{ if(typeof useFastExit==='function') useFastExit(); }});
    ...
    else if(b.kind==='crypt' && !b.noRead) cand.push({type:'inter',x:b.x,y:b.y+1,r:1.4,range:2.1,go:()=>readLore('crypt')});
