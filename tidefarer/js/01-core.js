...
/* Per-effect toggles for Performance mode (checkboxes in the pause menu).
   Defaults tuned on a real weak GPU (Snapdragon): the slow ones are the
   FULL-SCREEN gradient/blend fills (grade, vignette, shore foam, cloud shadows)
   - off by default. The cheaper image/point draws (bloom, night lighting,
   particles, gore, fireflies, birds) stay on. Persisted per-effect. In normal
   (non-perf) detail everything renders regardless - fxOn() short-circuits. */
const FX = { grade:0, vignette:0, foam:0, cloudShadows:0, fog:0,
             bloom:1, lighting:1, particles:1, decals:1, fireflies:1, birds:1 };
