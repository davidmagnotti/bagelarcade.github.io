  ...
  if(away){
    ...
    if(profile){
      ...
      if(expr==='hurt'){
        ...
        // gentle upward eye-curves (a warm squint) - softer stroke than before
        g.strokeStyle='#2e2418'; g.lineWidth=1.5; g.lineCap='round';
        g.beginPath();
        g.arc(-eSp+fx,eyeY+0.4,2.1,Math.PI*1.2,Math.PI*1.8);
        g.moveTo(eSp+fx+2.1*Math.cos(Math.PI*1.2),eyeY+0.4+2.1*Math.sin(Math.PI*1.2));
        g.arc( eSp+fx,eyeY+0.4,2.1,Math.PI*1.2,Math.PI*1.8);
        g.stroke(); g.lineCap='butt';
      } else if(blink){
        ...
        for(const e of [-1,1]){
          ...
          g.beginPath(); g.arc(ex2-0.5,eyeY-0.7,0.5,0,TAU); g.fill();
        }
      }
      // filled tapered brows, tilted by mood. Lifted ~0.8u off the eyes and set a
      // touch softer/thinner than before, so the resting face reads open & friendly
      // instead of a low, heavy glare.
      g.save(); g.translate(0,-0.8);
      g.fillStyle='#3a2d1f';
      const bT= (o.expr==='battle')? 1.6 : (o.expr==='hurt')? -1.3 : (o.expr==='happy')? -0.7 : 0;
      for(const e of [-1,1]){
        g.beginPath();
        g.moveTo(e*(eSp+2.5)+fx,-6.3-bT*0.4);
        g.quadraticCurveTo(e*eSp+fx,-8.0+bT*0.3, e*(eSp-2.5)+fx,-6.75+bT);
        g.quadraticCurveTo(e*eSp+fx,-7.15+bT*0.3, e*(eSp+2.5)+fx,-5.95-bT*0.4);
        g.closePath(); g.fill();
      }
      g.restore();
      ...
      if(o.expr==='battle'){
        ...
        // a soft, closed-mouth smile - warm and gentle, not a manic open grin
        // (the old dark open-mouth cavity read as creepy on the small face)
        g.lineWidth=1.3; g.strokeStyle='rgba(120,66,44,0.85)';
        g.beginPath();
        g.moveTo(fx*0.4-2.3,4.1); g.quadraticCurveTo(fx*0.4,5.8, fx*0.4+2.3,4.1);
        g.stroke();
      } else {
  ...
  if(!o.hat && !helm2 && o.hairstyle!=='bald'){
    if(profile){
      ...
      if(o.fem){
        ...
        g.fillStyle=hgr;
        g.beginPath();
        g.moveTo(-HR*0.58,16.8);                              // left hair tip
        g.quadraticCurveTo(-HR*0.96,8.4, -HR*0.96,-1.4);      // outer edge of the left fall, a touch wider
        g.quadraticCurveTo(-HR*0.98,-8.4, -HR*0.64,-13.2);    // sweep up to the left temple
        g.quadraticCurveTo(-HR*0.34,-15.9, 0,-16.1);          // over the crown, left half - a flatter dome that follows the skull
        g.quadraticCurveTo(HR*0.34,-15.9, HR*0.64,-13.2);     // over the crown, right half
        g.quadraticCurveTo(HR*0.98,-8.4, HR*0.96,-1.4);       // down to the right temple
        g.quadraticCurveTo(HR*0.96,8.4, HR*0.58,16.8);        // outer edge of the right fall to its tip
        g.quadraticCurveTo(HR*0.46,11.0, HR*0.62,2.2);        // inner edge of the right fall, tucked WIDER off the cheek so the eye reads
        g.quadraticCurveTo(HR*0.68,-2.6, HR*0.72,-4.6);       // up to the hairline
        g.quadraticCurveTo(5.6,-7.3, 4.9,-4.9);               // fringe: soft points across the brow, lifted a touch off the eyes
        g.quadraticCurveTo(3.3,-7.1, 1.7,-4.7);
        g.quadraticCurveTo(0.1,-7.5, -1.7,-4.8);
        g.quadraticCurveTo(-3.3,-7.1, -4.9,-4.9);
        g.quadraticCurveTo(-5.6,-7.3, -HR*0.72,-4.6);         // left end of the hairline
        g.quadraticCurveTo(-HR*0.68,-2.6, -HR*0.62,2.2);      // down beside the left cheek, WIDER
        g.quadraticCurveTo(-HR*0.46,11.0, -HR*0.55,16.8);     // inner edge of the left fall, back to the tip
        g.closePath(); g.fill();
        g.strokeStyle=OUT; g.lineWidth=1.8; g.stroke();
        g.fillStyle='rgba(255,250,235,0.20)';                 // soft top-light on the crown
        g.beginPath(); g.ellipse(-3.4,-10.2,3.7,1.7,-0.3,0,TAU); g.fill();
        g.strokeStyle=shade(hc,-12); g.lineWidth=1.1; g.lineCap='round';   // a couple of strand lines for depth
        for(const e of [-1,1]){
          g.beginPath();
          g.moveTo(e*HR*0.80,-9);
          g.quadraticCurveTo(e*HR*0.86,3, e*HR*0.70,13.5);
          g.stroke();
        }
        g.lineCap='butt';
      } else {
  ...
  return makeCanvas(sz,sz,(g,w,h)=>{
    ...
    switch(kind){
      case 'goo': g.fillStyle='#7fca6a'; g.beginPath(); g.ellipse(0,2,10,8,0,0,TAU); g.fill();
        ...
        g.fillStyle='#3e6b30'; g.beginPath(); g.arc(-3,3,1.2,0,TAU); g.arc(3,4,1,0,TAU); g.fill(); break;
