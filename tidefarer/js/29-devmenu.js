/* =====================================================================
   DEV / DEBUG MENU  -  a scratch panel for testing: teleport between
   islands, mark curses/bosses defeated, drive the story, and buff the hero.

   OFF BY DEFAULT and hidden from normal players. It only appears once you
   turn on "Developer menu" in Settings > Display (persisted). While enabled,
   open it with the small "DEV" tab in the top-left, or the ` (backtick) key;
   while disabled, neither the tab nor the shortcut does anything.
   ===================================================================== */
(function(){
'use strict';
...
/* ---- gated activation -----------------------------------------------------
   The dev menu is opt-in. Nothing (no tab, no panel, no working shortcut) is
   shown until the player enables it in Settings > Display. The choice persists
   in tf_dev, so a tester keeps it on across sessions and everyone else never
   sees it. */
function devEnabled(){ try{ return SafeStore.get('tf_dev')==='1'; }catch(e){ return false; } }
function removeDevUI(){ ['devMenu','devTab'].forEach(id=>{ const el=document.getElementById(id); if(el) el.remove(); }); }
function applyDevMode(){ if(devEnabled()) build(); else removeDevUI(); }   // build() makes the tab (panel stays hidden until opened)
function setDevMode(on){
  try{ SafeStore.set('tf_dev', on?'1':'0'); }catch(e){}
  applyDevMode(); syncDevToggle();
  if(typeof note==='function') note(on?'Developer menu enabled - tap DEV or press `':'Developer menu disabled');
}

window.addEventListener('keydown',(e)=>{
  if(!devEnabled()) return;   // the ` shortcut is inert unless the menu is turned on
  if(e.key==='`'||e.key==='~'){
...
function buildDevToggle(){
  ...
  document.getElementById('cfgDevOn').onclick=()=>setDevMode(true);
  document.getElementById('cfgDevOff').onclick=()=>setDevMode(false);
  syncDevToggle();
  return true;
}
