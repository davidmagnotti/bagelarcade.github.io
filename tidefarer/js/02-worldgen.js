...
let ZONES = {
  ...
  grove:  {x:96,y:50,r:6,  name:"Foss's Grove"}
};
...
function placeObjects(){
  ...
  // The Slime Meadow earns its name: a nest of slimes to drill the flow on (cancels,
  // dash-strike, riposte) - a good early training patch on the way east. (Rask's old grove
  // is Foss the woodcutter's now - see 04-data / 06-dialog.)
  if(typeof ZONES!=='undefined' && ZONES.meadow){ const mm=ZONES.meadow;
    const mpts=[[mm.x-3,mm.y-2],[mm.x+3,mm.y-1],[mm.x-2,mm.y+3],[mm.x+3,mm.y+2],[mm.x,mm.y-3],[mm.x+1,mm.y+3],[mm.x-4,mm.y+1]];
    for(let mi=0;mi<mpts.length;mi++){ const sp=findOpenNear(Math.round(mpts[mi][0]),Math.round(mpts[mi][1]),3); if(sp) spawnMob('slime',sp[0],sp[1]); }
  }
